...
Критерий поворотных точек. Сначала напишем функцию подсчета поворотных точек. На вход подается массив e (ряд остатков). Определеим количество поворотных точек j как 0. Обходим массив. Начинается отсчёт с первого элемента, заканчивая предпоследним иначе получим ошибку. Сравниваем элемент с двумя соседними, если он одновременно меньше или одновременно больще соседних - это повортная точка. Инкрементируем (увеличиваем) их количество (j) при каждом выполнении условия. В конце возвращаем количество найденных точек.
